...
function z() {
  ...
  cargo cov -- report --use-color --ignore-filename-regex='/rustc/|/.cargo/|.rustup/toolchains|/tests.rs' --instr-profile=res.profdata $(objects)
  popd > /dev/null
}
...
function s() {
  ...
  cargo cov -- show \
    --use-color \
    --ignore-filename-regex='/rustc/|/.cargo/|.rustup/toolchains|/tests.rs' \
