...
impl PrettyPrint for RelationalExpression {
        ...
        T: Write,
    {
        let mut work = |re: &RelationalExpression, se: &ShiftExpression, op, kind| {
            let (first, successive) = prettypad(pad, state);
            writeln!(writer, "{first}RelationalExpression: {self}")
                .and_then(|()| re.concise_with_leftpad(writer, &successive, Spot::NotFinal))
                .and_then(|()| pprint_token(writer, op, kind, &successive, Spot::NotFinal))
                .and_then(|()| se.concise_with_leftpad(writer, &successive, Spot::Final))
        };
...
impl RelationalExpression {
    ...
    pub fn has_call_in_tail_position(&self, location: &Location) -> bool {
        ...
        //      ShiftExpression
        //  1. Return HasCallInTailPosition of ShiftExpression with argument call.
        // RelationalExpression :
        ...
        //  1. Return false.
        match self {
            RelationalExpression::ShiftExpression(shift_expression) => {
                shift_expression.has_call_in_tail_position(location)
            }
            RelationalExpression::Less(..)
            | RelationalExpression::Greater(..)
            | RelationalExpression::LessEqual(..)
            | RelationalExpression::GreaterEqual(..)
            | RelationalExpression::InstanceOf(..)
            | RelationalExpression::In(..)
            | RelationalExpression::PrivateIn(..) => false,
        }
    }
}
